...
from ..algs.asam_utils import maybe_apply_asam_delta
...
class MiniGPTOutput(ModelOutput):
    ...
    asam_perturb_mask: torch.IntTensor = None
...
class MiniGPT4(Blip2Base):
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompt, return_ranges=False):
        if prompt:
            ...
            vision_range = (p_before_embeds.shape[1], p_before_embeds.shape[1] + img_embeds.shape[1])
            if return_ranges:
                return wrapped_img_embeds, wrapped_atts_img, vision_range
            return wrapped_img_embeds, wrapped_atts_img
        else:
            if return_ranges:
                return img_embeds, atts_img, (0, img_embeds.shape[1])
    ...
    def forward(self, samples):
        if samples['image'] is not None:
            ...
            vqa_prompt = '###Human: <Img><ImageHere></Img> '
            img_embeds, atts_img, vision_range = self.prompt_wrap(img_embeds, atts_img, vqa_prompt, return_ranges=True)
            ...
            vision_mask[:, vision_range[0]:vision_range[1]] = True
        ...
        asam_perturb_mask = targets.eq(-100) & attention_mask.bool()
        answer_mask = targets.ne(-100) & attention_mask.bool()
        prompt_mask = targets.eq(-100) & attention_mask.bool() & ~vision_mask
        samples["attention_mask"] = attention_mask
        samples["vision_mask"] = vision_mask
        samples["prompt_mask"] = prompt_mask
        samples["answer_mask"] = answer_mask
        inputs_embeds = maybe_apply_asam_delta(self, inputs_embeds, perturb_mask=asam_perturb_mask)
        ...
        return MiniGPTOutput(
            ...
            labels=targets,
            attention_mask=attention_mask,
            asam_perturb_mask=asam_perturb_mask,
