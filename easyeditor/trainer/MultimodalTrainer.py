...
from .algs.asam_utils import (
    asam_alignment_loss,
    asam_beta,
    asam_debug_grad_check,
    asam_enabled,
    gradient_diagnostics_for_params,
    normalized_alg_name,
)
...
class MultimodalTrainer(BaseTrainer):
    ...
    def edit_step(self, batch, training: bool):
        if normalized_alg_name(self.config) == "DSCA":
        ...
        # Do the edit
        start = time.time()
        edit_condition = batch if normalized_alg_name(self.config) == "ASAM_FT" else batch["cond"]
        edited_model, model_info = self.model.edit(batch["edit_inner"], edit_condition)
        ...
        if asam_enabled(self.config):
            asam_result = asam_alignment_loss(edited_model, self.config, batch["edit_inner"], return_info=True)
            asam_loss = asam_result.loss
            asam_info = asam_result.info
            if asam_debug_grad_check(self.config) and normalized_alg_name(self.config) == "ASAM_MEND":
                asam_info.update(gradient_diagnostics_for_params(
                    asam_loss,
                    self.model.outer_parameters(),
                    "asam/mend_outer",
                ))
            l_total_edit = l_total_edit + asam_beta(self.config) * asam_loss
        

        if training and normalized_alg_name(self.config) not in {"FT", "ASAM_FT"}:
        ...
        if asam_loss is not None:
            info_dict["loss/asam_align"] = asam_loss.item()
            info_dict["loss/asam_total"] = l_total_edit.item()
            info_dict["asam/beta"] = asam_beta(self.config)
            info_dict.update(asam_info)
