...
def get_model(config):
    if config.model_class == "BertClassifier":
        ...
        # for name, param in model.named_parameters():
        #     print(f"{name}: {param.shape}")
    elif config.model_name in {"llava-med", "llava_med"}:
        from .llava_med_models import LlavaMedForEditing

        model = LlavaMedForEditing(
            model_path=config.name,
            vision_tower_path=config.llava_med_vision_tower,
            model_name=getattr(config, "llava_med_model_name", "llava-med-v1.5-mistral-7b"),
            official_loader_source=getattr(config, "llava_med_loader_source", "third_party/LLaVA-Med"),
            device=config.device,
            dtype=getattr(config, "llava_med_dtype", "float16"),
            conversation_template=getattr(config, "llava_med_conversation_template", "mistral_instruct"),
        )
