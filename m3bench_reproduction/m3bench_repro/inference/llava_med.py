...
class LlavaMedAdapter(VLMAdapter):
    ...
    def prepare_inputs(self, image_path: str | Path, question: str, answer: str | None = None) -> dict[str, Any]:
        ...
        return {"prompt": prompt, "input_ids": input_ids.to(self.device), "images": image_tensor,
    ...
    def generate_with_result(
        ...
        with torch.inference_mode():
            output = self.model.generate(batch["input_ids"], images=batch["images"], **kwargs)
    ...
    def teacher_forced_loss(self, image_path: str | Path, question: str, target: str):
        ...
        labels[:, :prefix] = -100
        return self.model(input_ids=batch["input_ids"], images=batch["images"], labels=labels).loss
