...
def run(cfg):
    ...
    results=[];strict=False;torch.use_deterministic_algorithms(True)
    ...
    for n in (8,1):
        ...
        hook=MedTraceLayerHook(rt.get_module(training.LAYER),ex);hook.attach();r=dict(position=n,strict_deterministic_diagnostic=True,CUBLAS_WORKSPACE_CONFIG=os.environ.get('CUBLAS_WORKSPACE_CONFIG'),attention_implementation=getattr(rt.model.config,'_attn_implementation',None))
        try:
            ...
            g4=vector(scaled_regularizer.scaled_regularizer_gradients(v,params,scale=1048576,retain_graph=True));r['plateau']=diff(g4,g[0]);del f,v,g,g4
            if not strict:
