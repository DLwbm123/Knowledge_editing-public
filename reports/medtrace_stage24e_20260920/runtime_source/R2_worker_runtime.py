...
import os,sys,json,time,shutil,gc,hashlib
from collections import defaultdict
from pathlib import Path
from dataclasses import replace
ROOT=Path(os.environ.get('E_CODE_ROOT','<CODE_ROOT>'))
sys.path.insert(0,'/tmp');sys.path.insert(0,str(ROOT));sys.path.insert(0,str(ROOT/'reports/medtrace_stage22_20260919'))
...
def main(cfg):
    ...
    plan=read(p/'COMPILED_PLAN.json'); stream=read(common/'private/STREAM.json'); limit=int(os.environ.get('R2_N','19')); tasks=stream['tasks'][:limit]
    ...
    keys={}; row_by_q={query_id(r):r for t in tasks for role in ('H_fit','U_fit') for r in t[role]}
    row_by_q.update(plan.get('rows',{})); routers={n:MemoryRouter.from_state(dict(distance='euclidean',entries=[prior['routes'][e] for e in ids[:n]]),device=rt.device) for n in range(1,20)}
    ex=LowRankExpert(AsymmetricCPExpert(14336,4096,4),20260912,rank=4).to(rt.device).requires_grad_(False)
    global_teachers={}
    for _tt in tasks:
        for _tx in teachers_for(rt,common,cfg,_tt,record_for(_tt)):
            for _pid,_prow in plan.get('rows',{}).items():
                if _prow.get('image_sha256') == _tx[4].get('image_sha256') and _prow.get('question') == _tx[4].get('question'):
                    global_teachers[_pid] = _tx
    slot_by_t=defaultdict(list)
    for _slot in plan['slots']:
        if _slot['t'] <= limit: slot_by_t[_slot['t']].append(_slot)
    events=p/'EVENTS.jsonl'
    def event(kind, **kw):
        rec=dict(ts=time.time(),kind=kind,**kw)
        with events.open('a') as ef: ef.write(json.dumps(rec,ensure_ascii=False)+'\n'); ef.flush(); os.fsync(ef.fileno())

    def emit(arm,n,rec,bank,query_rows):
        event('INSERT_PREPARED',arm=arm,prefix=n,rows=len(query_rows),parameter_version=hashlib.sha256(str(sorted(bank.get('experts',{}).keys())).encode()).hexdigest())
        ...
        event('INSERT_GENERATED',arm=arm,prefix=n,rows=len(query_rows))
    ...
    arms=(os.environ.get('R2_ARM'),) if os.environ.get('R2_ARM') else ('B0','B1','B2')
    for arm in arms:
        ...
        for task in tasks:
            ...
            if arm in ('B1','B2') and n in slot_by_t:
                for slot in slot_by_t[n]:
                  event('MAINTENANCE_PREPARED',arm=arm,prefix=n,slot_id=slot.get('slot_id',f"t{n}"),expert=slot['expert'])
                  for j in sorted(set([slot['expert']])):
                    ...
                    try:
                        for step in range(20):
                            ...
                            masked = uq is not None and query_id(uq) in set(plan.get('U_masks',{}).get(str(n),[]))
                            u=teacher_map.get(query_id(uq)) if uq is not None else None
                            if uq is not None and u is None: raise RuntimeError('missing exact frozen U teacher binding')
                            update(rt,hook,ex,opt,source_batch(rt,record_for(target),target['native']),rt.build_edit_batch(replace(record_for(target),question=target['fit_questions'][0])),None if masked else u,h,extra_weight=.25,U_weight=0.0 if masked else .01)
                            event('MAINTENANCE_STEP',arm=arm,prefix=n,slot_id=slot.get('slot_id',f"t{n}"),expert=j,step=step,U_active=not masked,U_query=None if uq is None else query_id(uq))
        ...
        event('ARM_COMPLETE',arm=arm,N=limit)
        write(pub/f'ARM_{arm}.json',dict(arm=arm,status='GENERATED_NOT_SCORED',N=limit))
    write(pub/'GENERATED.json',dict(status='COMPLETE_NOT_SCORED',arms=list(arms),N=limit,phase='24E_R2'))
    event('RUN_COMPLETE',arms=list(arms),N=limit)
