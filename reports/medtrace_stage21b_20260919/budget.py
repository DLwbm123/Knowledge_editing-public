"""Inherited cumulative Stage21 wall-clock budget; failed attempts remain charged."""
...
def validate(auth,cfg):
    ...
    assert digest(auth)==cfg['authorization_binding'] and cfg['phase']=='FACT_FIXED_L31_DOWN'
...
def supervise(cfg):
    ...
    with (root/'private/supervisor.lock').open('a') as lock:
        ...
        assert ledger['sessions'][0]['seconds']==cfg['inherited_A_seconds']
...
def selfcheck():
    ...
    c=dict(gpu_uuid='u',gpu=0,phase='FACT_FIXED_L31_DOWN',authorization_binding=digest(a));validate(a,c)
