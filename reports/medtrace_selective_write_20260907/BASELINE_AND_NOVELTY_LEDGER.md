...
Read-only safetensors header inspection also succeeded: 69 tensors, 103,135,232 stored parameters; components `edit_extractor`, `input_extractor`, `instant_reps_norm`, `moegen_c`, `moegen_r`. Both generator `phi` tensors are [1,4,1024], residual-normalization width is 4096, and the visual sentinel prototype is [1,576,4096]. This confirms the complete checkpoint structure is present; it does not validate V4 end-to-end compatibility or clear training exposure.
