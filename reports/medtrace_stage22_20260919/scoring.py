"""Shared 2000-item follow-on judging budget; all stages count together."""
import sys
...
def judge(root,records,name,category='22B'):
    ...
    for offset in range(0,len(novel),60):
        ...
        source=dict(scope='STAGE22_'+label,records=chunk);inputpath=p/(label+'_JUDGE_INPUT.json');bundle=p/(label+'_JUDGE')
