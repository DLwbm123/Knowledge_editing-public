...
def run(cfg):
    ...
    for n in reg['mechanical_positions']:
        ...
        try:
            ...
            refg=torch.autograd.grad(reference,(zl,fl));plateau=comparison(gradients[4096],gradients[65536]);stable=plateau['relative_difference'] is not None and plateau['relative_difference']<=.01 and plateauau_cos(plateau)
            numeric=dict(position=n,D_x=float(dx.detach()),D_y=float(dy.detach()),reg=float(r.detach()),gx=stats(gx),gy=stats(gy),gx_plus_gy=stats(gx+gy),component_sum_vs_true=comparison(gx+gy,gradients[1]),scaled_recovery_vs_component_sum=comparison(gradients[65536],gx+gy),combined={str(s):stats(g) for s,g in gradients.items()},boundaries=layers,pooled_FP64_reference=[stats(g) for g in refg],reference_scope='partial derivatives at identical rounded pooled values; not a full CUDA Jacobian reference',scale_plateau=plateau,scale_stable=stable,kernels=regularizer.kernel_diagnostics(f))
            full_values={k:v.detach().clone() for k,v in f.items()};hidden_grad=gradients[65536].clone();actual=torch.cat([o['writer_inputs'][0] for o in boundary]).detach().float()
            private.append(dict(position=n,masks=m));del targets,gs,f,r,dx,dy,boundary,refg,zl,fl,reference
            parity={}
            for path in ('full','hidden'):
                stamp=clock();pf,_=regularizer.capture(rt,hook,batches,path=path);pr,*_=regularizer.loss(pf);pg=vec(scaled_regularizer.scaled_regularizer_gradients(pr,params,retain_graph=False));duration=clock()-stamp
        ...
        recovered=numeric['combined']['65536']['finite'] and numeric['combined']['65536']['nonzero']>0
