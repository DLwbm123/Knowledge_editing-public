...
def update(runtime,hook,expert,optimizer,native,fit,teacher,extra=None,extra_weight=1.,U_weight=.01,diagnostic=False,regularize=False,batches=None,observer=None):
    ...
    if regularize:
        ...
        if diagnostic:
            ...
            low=torch.cat([v.flatten() for v in scaled_regularizer_gradients(reg,params,scale=1048576.,retain_graph=True)]).double()
            high=g.double();den=float(high.norm());relative=float((low-high).norm()/high.norm()) if den else None
            cosine_scale=float(low@high/(low.norm()*high.norm())) if den and float(low.norm()) else None
            regularization['scale_plateau']=dict(relative_difference=relative,cosine=cosine_scale,scales=[1048576,16777216])
            if not torch.isfinite(high).all() or not torch.isfinite(low).all() or relative is None or cosine_scale is None or relative>.01 or cosine_scale<.999:raise FloatingPointError('Preregistered scale stability gate failed; preserve partial state')
            def reg_cosine(a,b):
...
def train(runtime,root,cfg,task,expert,branch,record,teachers,w0_hash,*,layer_id=30):
    ...
    binding=dict(task=task,branch=branch,W0=w0_hash,code=cfg['code_commit'],runtime=cfg['runtime_lock'],
        ...
        numeric_version='R2_FIXED_2POW24',regularizer=dict(lambda_x=.001,lambda_y=.001,sigma=1.,epsilon=1e-5,pooling='all valid tokens',observations=5,every_step=True),deterministic_backend=True,
    ...
    from regularizer import build_prefix_cache
    import regularizer
    regularizer.ACTIVE_CACHE=build_prefix_cache(runtime,batches)
    ...
    try:
        for step in range(step0+1,321):
            ...
            item=update(runtime,hook,expert,optimizer,batches[0],batches[fit_order[(step-1)%4]],teachers[(step-1)%len(teachers)],extra,extra_weight=weights['H'],U_weight=weights['U'],diagnostic=step in (1,80,160,320),regularize=True,batches=batches)
        ...
        hook.detach();regularizer.ACTIVE_CACHE=None
