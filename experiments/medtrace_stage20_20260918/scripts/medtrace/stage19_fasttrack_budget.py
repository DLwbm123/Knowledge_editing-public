...
def supervise(cfg):
    ...
    if cfg.get('pure_only'):
        sys.path.insert(0,str(Path(__file__).resolve().parents[2]))
        from scripts.medtrace.stage17_prepare import digest
        auth=read(root/'public/RUN_AUTHORIZATION.json')
        if (not auth['approved'] or not auth['pure_only'] or digest(auth)!=cfg['authorization_binding']
                or str(auth['physical_gpu'])!=str(cfg['gpu']) or auth['gpu_uuid']!=cfg['gpu_uuid']
                or auth['gpu_seconds_limit']!=28800 or auth['new_judgment_limit']!=(2000 if cfg.get('stage')==20 else 300)):
            raise ValueError('Independent pure-run resource authorization changed')
    with (root/'private/supervisor.lock').open('a') as lock:
        ...
        if cfg.get('phase','').startswith('ablation'):
            available=min(available,3600-sum(s['seconds'] for s in ledger['sessions'] if s['phase'].startswith('ablation')))
        if available < 60:
