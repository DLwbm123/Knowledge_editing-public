...
def train_one(runtime, root, cfg, task, record, branch):
    if cfg.get('ablation'):
        from scripts.medtrace.stage20_runtime import train_ablation
        return train_ablation(runtime,root,cfg,task,record,branch)
    ...
    if selected.exists():
        ...
        if branch == 'C_FACT':
            ...
            if cfg.get('stage')==20:
                ...
                selection['diagnostics']=hsic_diagnostics(features,selection)
            del features
    ...
    if cfg.get('stage')==20 and task['order']<=11:
        from scripts.medtrace.run_selective_write import save
        save(directory/'W0_R4.pt',dict(expert=expert.state_dict(),W0=w0,layer_id=layer,task=digest(task),code=cfg['code_commit']))
...
def campaign(cfg):
    ...
    arms=('A',) if cfg.get('noh_only') else ('A','B')
    if cfg.get('noh_only') and not cfg.get('ablation'):raise ValueError('Single-arm recovery is restricted to registered NO_H')
    if cfg.get('pure_only') and (stream['track']!='P' or any(not t['H_fit'] or not t['U_fit'] for t in tasks)):
        raise ValueError('Pure run cannot contain shared/no-H background')
    coverage=lambda n:n if stream['track']=='P' else min(11,n)
    ...
    def generated(arm, inserted, row, mode):
        ...
        if cfg.get('stage')==20:
            result['writer_layer']=banks[arm][selected].get('layer_id',21) if selected else None
        with outputs.open('a') as f: f.write(json.dumps(result)+'\n')
        return result

    def endpoint(inserted,final=False):
        rows = {query_id(t['native']):t['native'] for t in tasks[:len(inserted)]}
        rows.update({query_id(r):r for r in stream['core_rows']})
        if cfg.get('stage')==20 and not cfg.get('ablation') and (len(inserted)>=50 or final):
            rows.update({query_id(r):r for r in stream['new_rows']+stream['positive_rows']})
        for arm in arms:
            for row in rows.values(): generated(arm, inserted, row, 'endpoint')
        write(root/'public'/f'PREFIX_{len(inserted):03d}.json', dict(status='GENERATED_NOT_SCORED', N=len(inserted), K=coverage(len(inserted)), anchor_count=min(11,len(inserted)), track=stream['track'], outputs_per_arm=len(rows)))
    ...
    for task in tasks[len(inserted):]:
        ...
        reserve=300 if cfg.get('ablation') else 7200
        if inserted and cfg['gpu_deadline_epoch']-time.time() < reserve+estimate:
        ...
        if cfg.get('ablation'):
            prior=torch.load(Path(cfg['main_run'])/'private/BANKS.pt',map_location='cpu',weights_only=False)['routes'][edit]
            if not torch.equal(prior['key'],entry['key'].cpu()) or prior['radius']!=entry['radius']:raise ValueError('Ablation route differs from main pure11')
        shared = stream['track']=='S' and edit not in anchors
        branch = 'C_NO_H' if shared or cfg.get('ablation') else 'C_FACT'
        banks['A'][edit] = train_one(runtime, root, cfg, task, record, branch)
        if shared:
            banks['B'][edit] = dict(banks['A'][edit])
        elif cfg.get('noh_only'):
            pass  # Ineligible EXTRA remains the preserved one-edit partial bank; never deploy it as pure11.
        elif cfg.get('ablation'):
            banks['B'][edit] = train_one(runtime,root,cfg,task,record,'C_EXTRA')
        else:
        ...
        insertion = [generated(arm,inserted,task['native'],'insertion') for arm in arms]
        if index in (1,12):
            # Actual saved/reloaded active outputs and Base OFF parity for the first pair and mixed bank.
            for arm, first in zip(arms, insertion):
        ...
        prefixes=(11,) if cfg.get('ablation') else (11,19,32,50,100) if cfg.get('stage')==20 else (11,50,100)
        if index in prefixes: endpoint(inserted,final=index==len(tasks)); completed_prefixes.append(index)
    ...
    if len(inserted) not in completed_prefixes: endpoint(inserted,final=True)
    elif cfg.get('stage')==20 and not cfg.get('ablation') and len(inserted)<len(tasks) and len(inserted)<50:
        # Early resource boundary at a registered prefix: retain core rows and append only missing final-panel consumers.
        for arm in ('A','B'):
            for row in stream['new_rows']+stream['positive_rows']:generated(arm,inserted,row,'endpoint')
    guard()
    ...
    write_new(root/'public/GENERATED.json',dict(status='SINGLE_ENDPOINT_GENERATED_NOT_SCORED' if cfg.get('noh_only') else 'PAIRED_ENDPOINTS_GENERATED_NOT_SCORED',arms=list(arms),N=len(inserted),K=coverage(len(inserted)),anchor_count=min(11,len(inserted)),track=stream['track'],prefixes=sorted(set(completed_prefixes+[len(inserted)])),planned_N=len(tasks),cleanup_permitted=False))
...
if __name__ == '__main__':
    ...
    elif config['phase'].startswith('ablation'):
        from scripts.medtrace.stage20_runtime import ablation_config
        campaign(ablation_config(config))
