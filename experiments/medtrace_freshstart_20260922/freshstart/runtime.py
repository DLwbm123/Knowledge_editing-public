...
ROOT = Path('/root/rivermind-data/job-622/state')
GPU = 'GPU-d99033e1-3ba0-576d-49a9-4b207b6572b7'
...
def gpu_session(purpose):
    ...
    with ledger_lock() as ledger:
        if any(x.get('ended_epoch') is None for x in ledger['gpu_sessions']):
            raise RuntimeError('Unclosed GPU session requires reconciliation')
