...
def campaign(cfg):
    ...
    write_new(root/'public/GENERATED.json',dict(status='PAIRED_ENDPOINTS_GENERATED_NOT_SCORED',N=len(inserted),K=coverage(len(inserted)),anchor_count=min(11,len(inserted)),track=stream['track'],prefixes=sorted(set(completed_prefixes+[len(inserted)])),planned_N=len(tasks),cleanup_permitted=False))
