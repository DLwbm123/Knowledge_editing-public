#!/usr/bin/env python3
"""One detached fixed-GPU chain, no recurring monitor or unrelated process control."""
import argparse
import os
...
def launch(args):
    ...
    gpu='1' if cfg['kind']=='MEDTRACE_STAGE14' else '0'
    assert cfg['kind'] in ('MEDTRACE_STAGE13R','MEDTRACE_STAGE14') and cfg['allowed_physical_gpus']==[int(gpu)]
    assert read(run/'private/TASKS.json')
    if cfg['kind']=='MEDTRACE_STAGE14':assert len(read(run/'private/TASKS.json'))<=22 and cfg['train_seconds']==10800 and cfg['wall_hours']==4
    ...
    common.GPUS.clear();common.GPUS[gpu]=cfg['gpu_uuids'][gpu]
    check=common.gpu_check(gpu);vf.atomic_json(run/'private/GPU_START_CHECK.json',check)
...
def coordinate(args):
    ...
    physical_gpu='1' if cfg['kind']=='MEDTRACE_STAGE14' else '0'
    common.GPUS.clear();common.GPUS[physical_gpu]=cfg['gpu_uuids'][physical_gpu]
    children={};exits={};intervals={}
    def execute(name,command,gpu=False,judge=False):
        env=common.environment(physical_gpu,judge=judge) if gpu else dict(os.environ,CUDA_VISIBLE_DEVICES='',OMP_NUM_THREADS='1')
        env.update(JOB_ENTRYPOINT=os.environ['JOB_ENTRYPOINT'],M3BENCH_FORMAL_ALLOWED_CUDA_VISIBLE_DEVICES=physical_gpu)
        ...
        children[name]=proc;intervals[name]=dict(start=time.time(),gpu=int(physical_gpu) if gpu else None)
        vf.atomic_json(run/'PIPELINE_PIDS.json',dict(coordinator=os.getpid(),children={k:p.pid for k,p in children.items()}))
        vf.atomic_json(run/'public/RUN_STATUS.json',dict(status=name.upper()+'_RUNNING',gpu=int(physical_gpu),actual_N=len(read(run/'private/TASKS.json')),publication='PENDING'))
    ...
    try:
        script=str(ROOT/('scripts/medtrace/stage14.py' if cfg['kind']=='MEDTRACE_STAGE14' else 'scripts/medtrace/stage13r.py'))
