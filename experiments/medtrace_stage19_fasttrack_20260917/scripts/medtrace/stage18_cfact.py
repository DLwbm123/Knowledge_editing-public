...
def artifact_binding(saved, expected, cfg):
    """Reuse an audited predecessor only when every non-code binding is unchanged."""
    if saved == expected: return saved
    if (cfg.get('mode') == 'STAGE19_FASTTRACK_TWO_ARMS'
            and saved.get('code') in cfg.get('approved_predecessor_commits', [])
            and dict(saved, code=expected['code']) == expected):
        return saved
    raise ValueError('Artifact binding changed beyond approved code lineage')


def extra_roles(fasttrack):
    # FASTTRACK has no C_EXTRA arm; unused G must not gate or enter its training.
    return ('H_fit',) if fasttrack else ('H_fit', 'G_fit')
...
def teachers_for(runtime,root,cfg,task,record):
    ...
    for row in task['U_fit']:
        ...
        binding=artifact_binding(cache['binding'],binding,cfg)
