...
def validate_task(t, *, fasttrack_branch=None):
    if fasttrack_branch not in (None, 'C_FACT', 'C_NO_H'):
        raise ValueError('Unsupported fasttrack training branch')
    ...
    for role in ('native','U_fit','H_fit','G_fit'):
        ...
        optional = (role == 'G_fit' and fasttrack_branch is not None) or (role == 'H_fit' and fasttrack_branch == 'C_NO_H')
        if not rows and not optional: raise ValueError('UNSUPPORTED: missing '+role)
        for row in rows:
    ...
    if fasttrack_branch is None and len(t['H_fit'])!=len(t['G_fit']): raise ValueError('H/G slot counts must match')
