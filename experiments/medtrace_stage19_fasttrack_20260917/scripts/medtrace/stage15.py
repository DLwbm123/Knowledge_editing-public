...
def train_steps(runtime, run, cfg, t, expert, name, cp_w0=False, record=None, layer_path=None):
    import torch
...
def initialize(runtime, run, cfg, t, record=None, seed_base=SEED, layer_path=None):
    import torch
    ...
    if not native_point.exists():
        ...
        result = run_event(runtime, event, {record.record_id:dict(raw_generated_token_ids=base['raw_token_ids'], model_answer_raw=base['raw_answer'])},
            native_point.parent, seed_base=seed_base, condition_limit=1e4, **({'layer_path':layer_path} if layer_path is not None else {}))
    ...
    if not a2_path.exists():
        budget(run, cfg)
        a2, result = train_condition(runtime, record, t['fit_questions'], native, A2, seed_base=seed_base, **({'layer_path':layer_path} if layer_path is not None else {}))
    ...
    train_steps(runtime, run, cfg, t, cp, 'CP_W0', cp_w0=True, record=record, layer_path=layer_path)
