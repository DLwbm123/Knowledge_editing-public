...
def judge(root,records,name,qualification=False):
    ...
    count=len(novel)
    if ledger['new_judgment_items_dispatched']+count>500 or qualification and ledger['qualification_items_dispatched']+count>100:
    ...
    if not qualification and name=='prefix11' and ledger['new_judgment_items_dispatched']+count+2*(len(read(p/'STREAM.json')['tasks'])+26)>500:
...
def report(root):
    ...
    for n in done['prefixes']:
        for arm in ('A','B'):
            ...
            subset=[d[q] for q in fixed];counts=Counter('BASE' if not r['route']['activated'] else 'ANCHOR' if r['route']['logical_edit_id'] in stream['anchors'] else 'BACKGROUND' for r in subset)
            item=dict(arm=arm,N=n,queries=len(subset),selected=dict(counts),switch_is_not_necessarily_error=True)
            if n!=done['prefixes'][0]:
                first=by[arm,done['prefixes'][0]];common=set(first)&set(d);transitions=Counter();switches=0
                for q in common:
                    a,b=correct(first[q]),correct(d[q]);switches+=first[q]['route']['logical_edit_id']!=d[q]['route']['logical_edit_id']
                    if a is not None and b is not None:transitions[str(bool(a))+'_to_'+str(bool(b))]+=1
                item.update(expert_switches=switches,common_inputs=len(common),scored_correctness_transitions=dict(transitions))
