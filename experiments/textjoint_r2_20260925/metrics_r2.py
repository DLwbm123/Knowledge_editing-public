...
def retention(task):
    return task in ('T1L','T2L','T2L_PRESSURE')
...
def summarize(rows,scores):
    ...
    for (arm,mode,prefix,task),rs in sorted(groups.items()):
        ...
        for r in rs:
            ...
            elif b is retention(task):eligible.append((r,v))
        ...
        result.append(dict(arm=arm,mode=mode,prefix=prefix,task=task,primary='Retention' if retention(task) else 'Fix',
...
def paired(rows,scores,arm,mode='single',prefix=1):
    ...
    for key,r in a.items():
        ...
        if base is not retention(r['task']):continue
