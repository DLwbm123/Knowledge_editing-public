...
def budget(run, cfg):
    if 'gpu_deadline_epoch' in cfg:
        from scripts.medtrace.stage19_fasttrack_budget import check
        check(cfg)
...
def train_steps(runtime, run, cfg, t, expert, name, cp_w0=False, record=None, layer_path=None, protection=None):
    import torch
    ...
    teachers = []
    if not cp_w0 and protection is None:
        for row in t['U']:
    ...
    hook = MedTraceLayerHook(runtime.get_module(layer_path or LAYER), expert); hook.attach(); started = time.time()
    try:
        total_steps=320 if cp_w0 else t.get('continuation_steps',320)
        assert total_steps in (80,160,320)
        for step in range(step0+1, total_steps+1):
            budget(run, cfg); optimizer.zero_grad(set_to_none=True); values = []; term_grads=[]; previous_grads=None
            for b in (batches[0], batches[fit_order[(step-1)%4]]):
                ...
                current_grads=[p.grad.detach().clone() for p in expert.parameters()]
                delta=current_grads if previous_grads is None else [a-b for a,b in zip(current_grads,previous_grads)]
                term_grads.append(float(torch.sqrt(sum(x.float().square().sum() for x in delta))))
            ...
            if protection is not None:
                kl_value = protection(runtime, hook, expert, step, name)
            u_grad=float(torch.sqrt(sum((p.grad-b).float().square().sum() for p,b in zip(expert.parameters(),previous_grads))))
            ce_norm=float(torch.sqrt(sum(g.float().square().sum() for g in previous_grads)))
            dot=float(sum((b.float()*(p.grad-b).float()).sum() for p,b in zip(expert.parameters(),previous_grads)))
            ce_u_cosine=dot/(ce_norm*u_grad) if ce_norm and u_grad else None
            ...
            clipped_norm=float(torch.sqrt(sum(p.grad.float().square().sum() for p in expert.parameters())))
            before_update=[p.detach().clone() for p in expert.parameters()]
            optimizer.step(); expert.normalize_factors_(verify_dense=False)
            assert all(torch.isfinite(p).all() for p in expert.parameters())
            update_norm=float(torch.sqrt(sum((p-b).float().square().sum() for p,b in zip(expert.parameters(),before_update))))
            curve.append(dict(step=step, native_ce=values[0], fit_ce=values[1], U_kl=kl_value,
                fit_index=fit_order[(step-1)%4], grad_norm=float(grad),clipped_grad_norm=clipped_norm,
                update_norm=update_norm,CE_gradient_norm=ce_norm,CE_U_cosine=ce_u_cosine,weighted_native_gradient_norm=term_grads[0],weighted_fit_gradient_norm=term_grads[1],weighted_U_gradient_norm=u_grad))
            if step%20==0:
                state=dict(expert=expert.state_dict(), optimizer=optimizer.state_dict(), step=step, condition=name,
                ...
                save(point, state)
                if name=='C_NO_H' and step in (80,160,320):
                    save(directory/f'step-{step}.pt', state)
                print('TRAIN', t['order'], name, step, flush=True)
        write(directory/'TRAINING.json', dict(status='COMPLETE', steps=total_steps, condition=name, forwards=forwards, tokens=tokens,
...
def initialize(runtime, run, cfg, t, record=None, seed_base=SEED, layer_path=None, protection=None):
    import torch
    ...
    if layer_path is not None:
        layer_binding = directory/'LAYER_BINDING.json'
        expected = dict(layer_path=layer_path, canonical_edit_id=t['canonical_edit_id'])
        if layer_binding.exists():
            if read(layer_binding)!=expected: raise ValueError('W0 layer/edit binding changed')
        else:
            if any(directory.iterdir()): raise ValueError('Unbound initialization cannot be reused at another layer')
            write(layer_binding, expected)
    ...
    if not native_point.exists():
        ...
        result = run_event(runtime, event, {record.record_id:dict(raw_generated_token_ids=base['raw_token_ids'], model_answer_raw=base['raw_answer'])},
            native_point.parent, seed_base=seed_base, condition_limit=1e4, protection=protection, **({'layer_path':layer_path} if layer_path is not None else {}))
    ...
    if not a2_path.exists():
        budget(run, cfg)
        a2, result = train_condition(runtime, record, t['fit_questions'], native, A2, seed_base=seed_base, protection=protection, **({'layer_path':layer_path} if layer_path is not None else {}))
    ...
    train_steps(runtime, run, cfg, t, cp, 'CP_W0', cp_w0=True, record=record, layer_path=layer_path, protection=protection)
