#!/usr/bin/env python3
"""Detached bounded Stage11 inference/Judge chain on explicitly authorized GPUs."""
import argparse
import os
...
from scripts.medtrace import stage11 as s
...
def launch_chain(args):
    run=args.run_root
    if (run/'PIPELINE_PIDS.json').exists():raise FileExistsError('Stage11 already launched')
...
def coordinate(args):
    ...
    def wait(names,limit):
        while any(children[n].poll() is None for n in names):
            ...
            gpu_limit=13*3600 if limit==6.5*3600 else 16*3600
    ...
    try:
        script=str(ROOT/'scripts/medtrace/stage11.py')
        available=[]
        for gpu in (0,1):
            try:
                if common.gpu_check(str(gpu))['free_mib']>=24576:available.append(gpu)
            except RuntimeError:pass
        if not available:raise RuntimeError('No authorized GPU has sufficient memory; no monitoring created')
        names=[]
        for part,gpu in enumerate(available):
            name='part'+str(part);names.append(name)
            launch(name,[sys.executable,script,'worker','--run-root',str(run),'--part',str(part),'--parts',str(len(available))],gpu)
        s.vf.atomic_json(run/'public/RUN_STATUS.json',dict(status='TRAINING_AND_GENERATION_RUNNING',gpus=available,publication='PENDING'))
        wait(names,6.5*3600)
        if any(exits[n]!=0 for n in names):raise RuntimeError('Affected partition failed; preserve all artifacts for recovery')
        ...
        s.vf.atomic_json(run/'public/RUN_STATUS.json',dict(status='ATTENTION_REQUIRED',error=str(exc),exit_codes=exits,publication='PENDING',training_state='inspect per-endpoint ledger'))
        raise
