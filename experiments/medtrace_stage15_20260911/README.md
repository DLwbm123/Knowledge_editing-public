...
[Completed results and limitations](../../reports/medtrace_stage15_20260911/GPT_PRO_REVIEW.md).
Both implemented branches completed 200 edits and all 2,426 Judge tuples were scored.
No legal H support was available for C_FACT or C_EXTRA_QA. The frozen primary RC
preserved locality but reduced cross-image generality to 1.22%; R0 remained near 96%.
Do not substitute the secondary R0 result for the primary RC result.

Training/generation/Judge execution: `c6519359b6d5646b4908526c5b1d37e5a753abc7`.
Research result and read-only summary: `a978d63`.
Dependencies are reused from the previously published Stage14 source snapshot.
The exact cohort requires the private exposure ledgers and authorized image bundles;
this source snapshot is not a standalone distribution of the dataset or models.
Inherited environment paths in these public dependencies are redacted. Configure
authorized local paths and GPU identities before using the runner.
