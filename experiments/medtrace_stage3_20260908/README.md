...
The user subsequently authorized GPU0/1 as well as GPU2/3. Scheduling amendment `9ffae2c` adopts the original workers and adds two workers on the same queue; frozen scientific inputs and methods remain unchanged. The detached coordinator retains the original 24-hour wall / 48 GPU-hour bounds, accounting for all four cards and reserving closure capacity by ending generation at 40 GPU-hours or 20 wall hours, whichever comes first. The Judge phase uses one GPU. No additional monitoring process is started. Final public delivery will be checked when the user next requests progress after completion. No novelty, SOTA, clinical-safety or full-M3Bench claim.
