#!/usr/bin/env python3
"""Detached bounded Stage10 inference/Judge chain on explicitly authorized GPUs."""
import argparse
import os
...
from scripts.medtrace import stage10 as s
...
def launch_chain(args):
    run=args.run_root
    if (run/'PIPELINE_PIDS.json').exists():raise FileExistsError('Stage10 already launched')
...
def coordinate(args):
    ...
    def wait(names,limit):
        while any(children[n].poll() is None for n in names):
            ...
            gpu_limit=6*3600 if limit==6*3600 else 12*3600
    ...
    try:
        script=str(ROOT/'scripts/medtrace/stage10.py')
