# MedTRACE Stage12 V2 focused source snapshot

[Stage12 V2 results, costs and limits](../../reports/medtrace_stage12_v2_20260911/GPT_PRO_REVIEW.md). Training source bfd7bf7; CPU-only mixed CSV repair aa328ee; research branch medtrace-stage12-20260911. New entrypoints: scripts/medtrace/stage12.py, coordinate_stage12.py; shared stage11_worker.py has a Stage12-only no-H branch. Other source files are inherited published dependencies. Existing authorized private Stage11 artifacts and earlier runtime/route/Judge bindings are required; this is not a standalone dataset distribution.

15/15 new no-H trajectories complete; Judge1244/1244 and8 adapter branch replays passed. H supervision improves forced joint correctness while preserving native edits, with a challenge tradeoff; fixed RC returns all H/U to Base and makes aggregate system results equal across the three writers. No next experiment. Private QA/images/weights/tokens/activations/Judge mappings excluded. CPU report/deployment checks: 2 passed after export repair; no training or Judge recomputation.

## Inherited Stage11 dependency provenance
