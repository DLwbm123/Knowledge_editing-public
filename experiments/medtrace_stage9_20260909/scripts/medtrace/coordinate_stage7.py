#!/usr/bin/env python3
"""Detached bounded Stage7 inference/Judge chain on explicitly authorized GPUs."""
import argparse
import os
...
def launch_chain(args):
    run=args.run_root
    if (run/'PIPELINE_PIDS.json').exists():raise FileExistsError('Stage7 already launched')
...
def coordinate(args):
    ...
    try:
        script=str(ROOT/'scripts/medtrace/stage7.py')
        ...
        if not wait(['first'],6*3600):raise RuntimeError('first edit interface/constraint check needs attention; retain formal development result')
        first=s.read(run/'private/edits/e00/result.json')
        if first['status']!='COMPLETE':raise RuntimeError('first edit not complete')
        for part in ('0','1'):
            launch('part'+part,[sys.executable,script,'worker','--run-root',str(run),'--part',part],int(part))
        s.vf.atomic_json(run/'public/RUN_STATUS.json',dict(status='REMAINING_EDITS_RUNNING',new_sgd_training=0,gpus=[0,1],publication='PENDING'))
        wait(['part0','part1'],6*3600)
