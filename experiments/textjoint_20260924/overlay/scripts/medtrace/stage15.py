    ...
    try:
        for step in range(step0+1, 321):
            ...
            curve.append(dict(step=step, native_ce=values[0], fit_ce=values[1], U_kl=kl_value,
                fit_index=fit_order[(step-1)%4], grad_norm=float(grad),weighted_native_gradient_norm=term_grads[0],weighted_fit_gradient_norm=term_grads[1],weighted_U_gradient_norm=u_grad))
            if step%20==0:
