...
def _write_preflight(out_dir: Path, *, hparams_path: Path, data_path: Optional[Path], image_root: Path, test_status: Dict[str, Any]) -> Dict[str, Any]:
    ...
    lines = ["# CURE 10-Edit Model-Known Preflight", "", f"- Status: `{payload['status']}`", f"- Python: `{sys.executable}`", "", "## Checks", ""]
...
def _build_candidate_records(
    *,
    ...
    for idx in range(1, int(candidate_count) + 1):
        ...
        record_id = f"synthetic-10edit-{idx:02d}"
        ...
        copied.update(
            {
                ...
                "pred": old_answer,
                "alt": f"synthetic-code-10edit-{idx:02d}",
                "erase_answer": old_answer,
                "old_answer": old_answer,
                "new_answer": f"synthetic-code-10edit-{idx:02d}",
                "replacement_answer": f"synthetic-code-10edit-{idx:02d}",
    ...
    candidate_path = raw_dir / "engram_replacement_10edit_modelknown_candidates.json"
...
def _filter_model_known_records(
    *,
    ...
    for record in candidate_records:
        ...
        valid = bool(image_resolved and x_minus_nonempty and _metric_available(old_raw) and _metric_available(ref_raw))
        rows.append(
            {
                "record_id": record_id,
                "valid": valid,
                ...
                "failure_reason": None if valid else "failed image/reference/token/finite pre-edit filter",
            }
        )
    valid_rows = [row for row in rows if row["valid"]]
    valid_rows_sorted = sorted(valid_rows, key=lambda row: float(row["old_answer_nll"]))
    ...
    report = {
        ...
        "rows": rows,
        "selection_rule": "valid finite old/reference NLL records sorted by lowest old_answer_nll",
    ...
    if report["status"] != "pass":
        ...
        return [], {}, report
    final_path = out_dir / "synthetic_root" / "data" / "medmkeb" / "raw" / "engram_replacement_10edit_modelknown.json"
...
def _write_stop_report(out_dir: Path, reason: str, payload: Dict[str, Any]) -> None:
    lines = [
        "# Final CURE 10-Edit Model-Known Report",
        "",
        f"- Status: `stopped`",
    ...
    (out_dir / "FINAL_CURE_10EDIT_MODELKNOWN_REPORT.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def _run_nonseq_10(
    *,
    ...
    checks = {
        "positive_new_answer_edits_at_least_8_of_10": int(cure.get("positive_new_answer_edits") or 0) >= 8,
...
def _aggregate_sequential_step_10(rows: List[Dict[str, Any]], method: str, step: int) -> Dict[str, Any]:
...
def _run_sequential_10(
    *,
    ...
    summary_rows = [
        _aggregate_sequential_step_10(rows, method, step)
        for method in ["C_engram_projected_tiny_lora", "E_cure_dual_projected_tiny_lora"]
    ...
    basic_checks = {
        ...
        "locality_damage_records_lte_1": int(cure.get("locality_damage_records") or 0) <= 1,
...
def _write_final_report(
    ...
    *,
    ...
    decision = "C. CURE fails 10-edit. Do not scale; inspect data filtering, beta/gamma/cache policy, or projection order."
    if nonseq_status == "pass" and sequential_status == "pass":
        decision = "A. CURE passes 10-edit nonseq and sequential gates and remains Pareto-promising. Next gate: 20-edit non-PHI/model-known validation."
    elif nonseq_status == "pass" and sequential_status in {"partial", "pass"}:
        decision = "B. CURE passes 10-edit but no longer clearly improves over ENGRAM-projected LoRA. Keep both C and E for 20-edit; treat CURE as conservative variant."
    lines = [
        "# Final CURE 10-Edit Model-Known Report",
        "",
        "## Motivation",
        ...
        "- This gate tests whether the same configuration survives a 10-edit model-known non-PHI validation.",
        "",
        "## Data",
        "",
        ...
        "- Filtering used finite old-answer/reference NLL and resolved image/reference paths.",
        "",
        "## Methods",
        "",
        "- `A_no_edit`",
        ...
        "",
        "| method | mean new decrease | mean old increase | mean reference delta | positive new | positive old erase | locality damage | rollback | match | nan/inf |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    ...
    lines.extend(
        [
            ...
            f"- average mask keep ratio: `{_format(diag.get('average_mask_keep_ratio'))}`",
            f"- projection norm ratio: `{_format(diag.get('projection_norm_ratio'))}`",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the 10-edit model-known non-PHI CURE-MedEdit gate.")
    ...
    parser.add_argument("--output-dir", default="outputs/cure_mededit_10edit_modelknown")
    ...
    parser.add_argument("--record-count", type=int, default=10)
    ...
    data_file = out_dir / "synthetic_root" / "data" / "medmkeb" / "raw" / "engram_replacement_10edit_modelknown.json"
    ...
    _write_final_report(out_dir, data_report=data_report, nonseq=nonseq, sequential=sequential, generation=generation)
    return 0
